...
int main() {
    ...
    ToolKit::takeTime([&ti,&f,&cr]() {
        for(int i= 0; i < 1; i++) {
            ConfigurationBulk conf= ConfigurationBulk(cr, f, ti);
            ...
            for(int m= ti.num_solutes+1; m < conf.getNMolec(); m++) {
                ...
                if(conf.getMolec(m).getClassification() == ConfigurationBulk::CLASSIFICATION_D3_MOLECULE) cuenta_D3++;
                if(conf.getMolec(m).getClassification() == ConfigurationBulk::CLASSIFICATION_D5_MOLECULE) cuenta_D5++;
                if(conf.getMolec(m).getClassification() == ConfigurationBulk::CLASSIFICATION_TA_MOLECULE) cuenta_TA++;
                if(conf.getMolec(m).getClassification() == ConfigurationBulk::CLASSIFICATION_TB_MOLECULE) cuenta_TB++;
                cuenta_total++;
            }
