...
inline Real distancePBC(const Vector& a, const Vector& b, const Vector& box) {
	...
	dx-= box.x * std::floor(dx / box.x + static_cast<Real>(0.5));
	dy-= box.y * std::floor(dy / box.y + static_cast<Real>(0.5));
	dz-= box.z * std::floor(dz / box.z + static_cast<Real>(0.5));
...
inline Real getAngle(Vector c1, Vector c2, Vector c3, Vector bounds) {
	...
	cos_angle= std::max(static_cast<Real>(-1.0), std::min(static_cast<Real>(1.0), cos_angle)); // Clamp value to [-1, 1] to avoid NaNs due to floating point errors
